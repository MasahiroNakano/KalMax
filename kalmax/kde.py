...
def kde(
    ...
    return_position_density : bool
        If True, this also returns the position density (the denominator of the KDE) at each bin position.


    Returns
    -------
    kernel_density_estimate : jnp.ndarray, shape (N_neurons, N_bins)
    position_density : jnp.ndarray, shape (N_neurons, N_bins) (only if return_position_density is True)
    """
    assert bins.ndim == 2
    ...
    for i in range(N_batchs):
        ...
        spike_density_batch = jnp.where(
            jnp.isnan(spike_density_batch), 0, spike_density_batch
        )
    ...
    kernel_density_estimate = jnp.exp(
        jnp.log(spike_density) - jnp.log(position_density)
    ).T

    if return_position_density:
        print("Returning position density as well as kernel density estimate")
...
def poisson_log_likelihood(
    ...
    spikes_ = jnp.where(
        spikes == 0, 1, spikes
    )  # replace 0 spikes with 1s because 0! = 1
